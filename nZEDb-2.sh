#!/bin/bash
# Script Name: nZEDb Setup - Part 2
# Author: Jorge Pabon - PREngineer - pianistapr@hotmail.com
# License: Personal Use (1 device)
# IMPORTANT: RUN THIS SCRIPT SECOND!
# This Script contains the Pre-requisites
# Before the second reboot and installs the DB
# and nZEDb among other things.
#############################################
...
echo
echo -e $GREEN'nZEDb Server Setup - Part 2'$BLACK
echo 
...
echo -e $RED "THIS WILL PROMPT YOU FOR THE -MYSQL ROOT- PASSWORD YOU JUST CREATED!!"
MYSQL=`which mysql`
Q0="CREATE USER 'nzedb'@'%' IDENTIFIED BY 'YOUR_PASSWORD';"
Q1="GRANT ALL ON *.* TO 'nzedb'@'%' IDENTIFIED BY 'YOUR_PASSWORD';"
...
sudo echo 'server {
    ...
    # This is the root web folder for nZEDb, you shouldnt have to change this.
...
sudo mv nZEDb /etc/nginx/sites-available/
...
echo -e $YELLOW
echo -e "---> [Removing Random Apache...]"$BLACK
sudo apt-get purge apache* -y > /dev/null
sudo apt-get remove apache* -y > /dev/null
...
echo -e $YELLOW
echo -e "---> [Replacing PHP.ini Files...]"$BLACK
sudo echo '[PHP]
...
; recommend error reporting setting. Your production server shouldnt be wasting
; resources complaining about best practices and coding standards. Thats what
...
;                     from a bug in your code, but its possible that it was
;                     intentional (e.g., using an uninitialized variable and
;                     relying on the fact its automatically initialized to an
;                     empty string)
; E_STRICT          - run-time notices, enable to have PHP suggest changes
;                     to your code which will ensure the best interoperability
;                     and forward compatibility of your code
; E_CORE_ERROR      - fatal errors that occur during PHPs initial startup
; E_CORE_WARNING    - warnings (non-fatal errors) that occur during PHPs
;                     initial startup
...
; Its recommended that errors be logged on production servers rather than
...
; The display of errors which occur during PHPs startup sequence are handled
; separately from display_errors. PHPs default behavior is to suppress those
; errors from clients. Turning the display of startup errors on can be useful in
; debugging configuration problems. But, its strongly recommended that you
...
; the dot. PHPs default behavior is to leave these settings empty, in which
...
; String to output before an error message. PHPs default behavior is to leave
; this setting blank.
; http://php.net/error-prepend-string
; Example:
;error_prepend_string = "<span style=color: #ff0000>"

; String to output after an error message. PHPs default behavior is to leave
; this setting blank.
...
; When enabled, the ENV, REQUEST and SERVER variables are created when theyre
...
; Most likely, you wont want to disable this option globally. It causes $_POST
...
; set it to be empty.
;
; PHPs built-in default is text/html
...
; Always populate the $HTTP_RAW_POST_DATA variable. PHPs default behavior is
...
;
; PHPs default setting for include_path is ".;/path/to/php/pear"
...
; every request. PHPs default behavior is to disable this feature.
;cgi.nph = 1
...
; cgi.fix_pathinfo provides *real* PATH_INFO/PATH_TRANSLATED support for CGI.  PHPs
...
; Disable logging through FastCGI connection. PHPs default behavior is to enable
; this feature.
;fastcgi.logging = 0

; cgi.rfc2616_headers configuration option tells PHP what type of headers to
; use when sending HTTP response code. If its set 0 PHP sends Status: header that
...
; Define the anonymous ftp password (your email address). PHPs default setting
; for this is empty.
; http://php.net/from
;from="john@doe.com"

; Define the User-Agent string. PHPs default setting for this is empty.
...
[MySQL]
; Allow accessing, from PHPs perspective, local files with LOAD DATA statements
...
; variable in order to use PHPs session functions.
;
...
; the session id. We encourage this operation as its very helpful in combating
...
; introduces some serious security problems if not handled correctly. Its
; recommended that you do not use this feature on production servers. But you
; should enable this on development servers and enable the warning as well. If you
; do not enable the feature on development servers, you wont be warned when its
...
; On systems that dont have /dev/urandom but do have /dev/arandom, this will default to /dev/arandom
...
echo -e $YELLOW
echo -e "---> [Installing Media Processors...]"$BLACK
...
echo -e $YELLOW
echo -e "---> [Installing Pzip & PAR2...]"$BLACK
sudo apt-get install p7zip-full par2 -y > /dev/null
echo -e $GREEN
echo -e "DONE!"
...
echo -e $RED"MAKE SURE TO CONFIGURE THE ADMIN PORTAL BEFORE RUNNING THE NEXT PART!"$BLACK
